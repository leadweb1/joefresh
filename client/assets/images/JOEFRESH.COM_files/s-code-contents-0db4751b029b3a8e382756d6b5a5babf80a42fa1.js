...
var s_account="lcljoefresh-dev"

var s=s_gi(s_account)

s.dynamicAccountSelection=true

s.dynamicAccountList="lcljoefresh-prod=www.joefresh.com/ca,shop.joefresh.com/ca;lcljoefresh-prod-us=www.joefresh.com/us,shop.joefresh.com/us"
s.dynamicAccountMatch=window.location.host+window.location.pathname.substring(0,3)
...
s.linkInternalFilters="javascript:,www.joefresh.com,shop.joefresh.com,dev.joefresh.loblaw.ca,dev.joefresh.sapient.com,qa-joefresh.sapient.com,local.joefresh.com,qa2.joefresh.com,qa2.us.joefresh.com,lclqa.joefresh.com,lclqa1.joefresh.com,lclqa1.us.joefresh.com,vlkreccqa02:9002,10.62.178.45:9002,qa1.joefresh.com"
s.linkLeaveQueryString=false
s.linkTrackVars="None"
s.linkTrackEvents="None"

// timeparting config variables
s.dstStart="3/08/2015"; // update to the correct Daylight Savings Time start date for the current year.
s.dstEnd="11/1/2015"; // update to the correct Daylight Savings Time end date for the current year.
s.currentYear="2015"; // update to the current year (can be done programmatically).
...
function s_doPlugins(s) {
	...
	//if gnav is used
	if(s.getQueryParam('gnav')) {
	    s.eVar22 = decodeURIComponent(s.getQueryParam('gnav'))
	    s.events = "event32";
	}
	
	//exact target
	if(s.getQueryParam('rid')) {
		s.eVar24 = decodeURIComponent(s.getQueryParam('rid'));
	}
	
	/*Time Parting Uses GMT -5 time zone */
	s.eVar9 = s.getTimeParting('d','-5').toLowerCase(); // Set day
	s.eVar10 = s.getTimeParting('h','-5'); // Set hour
	s.eVar11 = s.getTimeParting('w','-5').toLowerCase(); // Set weekday
  s.eVar29 = 'D=t';
  var now = new Date();
  s.eVar42 = now.toLocaleDateString();
	
	/*Visit information*/
	s.prop9 = s.getDaysSinceLastVisit('s_lv');
	s.prop10 = s.getNewRepeat(90,'s_getNewRepeat').toLowerCase();//getNewRepeat //cookie will last 30 days
	s.prop11 = s.getVisitNum(); //Visit Number
	
	//apply getValOnce rules
	s.prop9 = s.getValOnce(s.prop9,'s_ev9');
	s.prop10 = s.getValOnce(s.prop10,'s_ev10');
	s.prop11 = s.getValOnce(s.prop11,'s_ev11');
...
s.visitorNamespace="joefresh"
s.trackingServer="metrics.joefresh.com"
s.trackingServerSecure = "smetrics.joefresh.com"
